---
title:  "[PR113] One-shot Pruning: SNIP"
...
$$\begin{align}\Delta L_j(\mathbf{w};\mathcal{D})&=L(1\odot \mathbf{w};\mathcal{D})-L((1-\mathbf{e}_j)\odot \mathbf{w};\mathcal{D}) \\ &\approx g_i(\mathbf{w},\mathcal{D})\end{align}$$

- (중간과정 생략) weight에 dependency가 적고 한번의 forward pass로 모든 connection을 평가할 수 있는  "connection sensitivity"를 정의함.

$$\begin{align}s_j=\frac{|g_j(\mathbf{w};\mathcal{D})|}{\sum_{k=1}^{m}|g_k(\mathbf{w};\mathcal{D})|}\end{align}$$
...
- 다음과 같은 순서로 pruning을 수행함.
    ...
    2. mini-batch sampling $\mathcal{D}^{b}=\{(\mathbf{x}_{i},\mathbf{y}_{i})\}^{b}_{j=1}$
